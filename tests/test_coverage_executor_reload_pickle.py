...
succeed where it had been silently returning None:
    ...
    it's not the same object as NoWires.radio_coverage.pool._init_cov_pool

Cause: ``_coverage_executor`` captured ``_init_cov_pool`` /
``_itm_worker_batch`` at module import time via ``from NoWires.radio_coverage.pool
import ...``. If anything subsequently replaced ``NoWires.radio_coverage.pool``
...
diverged from ``sys.modules["NoWires.radio_coverage.pool"].<name>``, and
...
def test_executor_handles_coverage_pool_reload(monkeypatch):
    ...
    """
    coverage_engine = importlib.import_module("NoWires.radio_coverage.engine")
    coverage_executor = importlib.import_module("NoWires.radio_coverage._executor")
    coverage_pool = importlib.import_module("NoWires.radio_coverage.pool")
    ...
    sys.modules["NoWires.radio_coverage.pool"] = coverage_pool
...
def test_executor_does_not_import_init_cov_pool_at_module_level():
    ...
    """
    src_path = os.path.join(PLUGIN_DIR, "radio_coverage/_executor.py")
