...
class P2PAlgorithm(QgsProcessingAlgorithm):
    ...
    def _show_profile_chart(
                ...
                def toggle_terrain(state):
                    toggle_state["terrain"] = bool(state)
                    update_visibility()
    
                def toggle_los(state):
                    toggle_state["los"] = bool(state)
                    update_visibility()
    
                def toggle_fresnel(state):
                    toggle_state["fresnel"] = bool(state)
                    update_visibility()
    
                def toggle_violation(state):
                    toggle_state["violation_band"] = bool(state)
                    update_visibility()
                ...
                cb_terrain.stateChanged.connect(toggle_terrain)
    
                cb_los = QCheckBox("LOS", toolbar)
                cb_los.setChecked(True)
                cb_los.stateChanged.connect(toggle_los)
    
                cb_fresnel = QCheckBox("Fresnel", toolbar)
                cb_fresnel.setChecked(True)
                cb_fresnel.stateChanged.connect(toggle_fresnel)
    
                cb_violation = QCheckBox("60% Band", toolbar)
                cb_violation.setChecked(True)
                cb_violation.stateChanged.connect(toggle_violation)
