...
- Fix scalar/vector formula divergence in `clutter/saalos.py` below-canopy path. The scalar path at line 128 computes `math.exp(1.0 / cch__meter - h_tx__meter)` which Python evaluates as `exp((1/cch) - htx)`. The vectorized path at line 282 computes `np.exp(1.0 / m_cch_h)` where `m_cch_h = np.maximum(cch - htx, 1e-10)`, i.e. `exp(1/(cch-htx))`. These are fundamentally different calculations yielding different results. Align both paths to the same formula (the scalar form matches the SAALOS derivation in the module docstring). Additionally, the vectorized path's `np.exp(1.0 / m_cch_h)` overflows to `inf` when `m_cch_h` reaches the 1e-10 clamp; add an overflow guard via `np.minimum(1.0 / m_cch_h, 700.0)`.
