...
- Fix proxy auth handler in `contour_pipeline.setup_proxy_opener` (`contour_pipeline.py:42-69`): (a) pass the actual proxy realm/host to `HTTPPasswordMgrWithDefaultRealm.add_password()` instead of `None` (today's `None` realm sends Basic-Auth credentials to *any* server that requests them, including the upstream tile host on a 407→401 chain); (b) derive the scheme from the `realm` URL instead of hardcoding `http://` in `proxy_base_url`, so HTTPS proxies aren't downgraded.
- Fix `algorithm_batch.processAlgorithm` unconditionally closing the user-supplied clutter grid in its `finally` block (`algorithm_batch.py:255-256`). `inp.clutter_grid` may be either a user-loaded `LandCoverGrid` (built by `extract_clutter_params` from `CLUTTER_RASTER`) or an auto-downloaded grid from `ensure_clutter_grid_for_area`; both are closed today. v1.5.1 fixed the same ownership confusion for Coverage Analysis and v1.5.0 fixed it for P2P via `owns_clutter_grid`; Batch was missed. Track ownership with an `owns_clutter_grid` flag in `_collect_batch_inputs` (set True only when the grid was auto-downloaded) and gate the close in `processAlgorithm` on that flag — mirroring `p2p_compute.py:133-137,294-298`.
- Fix the Coverage Analysis GDAL handle leak when the user supplies `CLUTTER_RASTER`. `extract_clutter_params` calls `LandCoverGrid.from_raster(raster_path)` at `shared_params.py:179`, which opens a fresh GDAL dataset that lands on `p.clutter_grid`. `algorithm_coverage._build_clutter_context` returns `owns_grid=False` because `clutter_grid_resolved is clutter_grid` (`algorithm_coverage.py:69-71`), so the `finally` block at `algorithm_coverage.py:268-270` deliberately skips the close. Every user-clutter-raster coverage run leaks one GDAL dataset until process exit. The v1.5.1 "don't close user-provided" guidance predates `extract_clutter_params` opening the raster itself — the dataset is now algorithm-loaded and safe (and required) to close, consistent with `comparison_panel.py:127-128,256-257` which already does. Track ownership the same way as the batch fix above.
- Fix `coverage_pool._init_cov_pool` re-registering `_final_cov_pool` on every initializer call (`coverage_pool.py:150`). The v1.5.3 change that made the initializer re-bindable across runs (instead of raising on reuse) left the `atexit.register(_final_cov_pool)` call inside, so under worker reuse (threading-enabled or any reinit) atexit handlers accumulate inside each worker process and the finalizer fires N times against state that's already None after the first pass. Either gate the register on a module-level "registered" flag or `atexit.unregister(_final_cov_pool)` before re-registering.
- Fix `_coverage_executor.execute_coverage_tasks` discarding partial multiprocessing counters when the pool raises and falls back to sequential (`_coverage_executor.py:140-143`). The sequential phase overwrites `pixels_failed` / `pixels_done` with its own return values:
  ```python
  cancelled, pixels_failed, pixels_done = _run_sequential(...)
  ```
  `_run_sequential` correctly skips already-computed pixels so the grid stays correct, but the "X/Y pixels failed" line from `compute_coverage` undercounts any failures that happened in the MP phase before the exception. Accumulate (`pixels_failed += seq_failed`, `pixels_done += seq_done`) rather than reassign.
- Fix the GDAL dataset leak in `contour_pipeline.download_and_merge_tiles` at `contour_pipeline.py:142`:
  ```python
  if gdal.Open(fn_clip) is None:
      continue
  ```
  On the success path the opened dataset is created and immediately discarded with no explicit `ds = None`, relying on CPython refcount-on-discard. Match the explicit-release pattern used in `tile_download_base.py:143-154` (`test_ds = gdal.Open(...); ok = test_ds is not None; test_ds = None; if not ok: continue`).
- Fix `NoWiresPlugin.run_pattern_preview` antenna-preview dialog leak (`nowires.py:252-256`). `self._pattern_preview_dialog = dlg` is not declared in `__init__` and not cleaned up in `unload()` — each menu invocation overwrites the previous reference without closing the prior dialog, and plugin unload leaves any open dialog dangling. Mirror the `_opacity_dialog` lifecycle at `nowires.py:85,186-188`: initialise to `None`, close-then-replace on each invocation, and close in `unload()`.
- Clamp `geo_bounds.coverage_bounds` results to valid lat range (`geo_bounds.py:8-24`). The longitude width already guards `cos(tx_lat)` for near-pole TX positions, but `tx_lat ± half_lat ± padding_deg` can still exceed ±90° (e.g. `tx_lat=89, radius_km=300` yields `north > 90`). The invalid bounds then flow into DEM bounds, the AOI shapefile, and `_haversine_grid`. Either clamp `(north, south)` to `[-90, 90]` and document the polar-region limitation, or raise a `QgsProcessingException` when the analysis area crosses a pole. Bundle with: replace the local `meters_per_deg_lat = 111320.0` at `geo_bounds.py:13` with an import of `METERS_PER_DEGREE_LAT` from `constants.py:3` (same edit area).
- Fix `coverage_params._add_pct_params` using `DEFAULT_TIME_PCT` for all three percentage parameters at `coverage_params.py:121`. The v1.5.2 fix corrected `p2p_params.py`, `batch_params.py`, and `comparison_add_params.py` to use `DEFAULT_LOCATION_PCT` and `DEFAULT_SITUATION_PCT` for the LOCATION_PCT / SITUATION_PCT registrations respectively, but `coverage_params` was missed. Harmless today because all three constants are 50.0 in `defaults.py:13-15`, but the parameters silently re-couple if the defaults ever diverge — and the file is the only remaining sibling that doesn't match the post-v1.5.2 convention. Split the loop so each `addParameter` call references the matching default.
- Fix `contour_overlay.py:103-106` releasing the hillshade dataset without flushing pyramid overviews. `hillshade_ds.BuildOverviews("AVERAGE", overview_levels)` is followed immediately by `hillshade_ds = None`, with no `hillshade_ds.FlushCache()` in between. Both other GDAL write paths in the codebase flush before release (`contour_smoothing.py:83`, `raster_io.py:41`); overview data may not reach disk before the dataset handle is finalized, leaving the pyramids stale or empty on the next open. Add `hillshade_ds.FlushCache()` before the `None` rebind.
- Fix `algorithm_contour.py:96-97` leaking a permanent temp directory when `get_temp_dir()` returns None. The fallback `tempfile.mkdtemp(prefix="nowires_contour_")` is captured into `self.temp_dir` with no registration for cleanup — the inline comment literally states "never cleaned". `get_temp_dir()` returning None is rare but reachable (the v1.5.1 fix added this fallback path); when it does fire, each contour run leaves behind a `nowires_contour_*` directory under the system temp root that survives QGIS shutdown. Route the fallback through `TempDirManager` (already used at `algorithm_contour.py:101` for per-run files), or register the path for atexit cleanup.
- Fix `batch_writer.write_batch_csv` missing CSV formula-injection sanitisation (`batch_writer.py:104-117`). The v1.4.0 fix that added `_csv_safe()` to `report_export.write_report_csv` was scoped to the coverage/P2P CSV report path only; the batch writer writes user-influenced fields (`point_id`, `status`) without any formula-character guard. A cell value starting with `=`, `+`, `@`, tab, or CR would be written verbatim, allowing spreadsheet formula injection on CSV open. Apply the same `_csv_safe()` prefix-or-escape logic (or import and reuse the function) to every cell written by `write_batch_csv`.
- Fix `ElevationGrid.__init__` zero-division on degenerate DEM rasters (`elevation.py:144-145`). `self.d_lat = (self.max_lat - self.min_lat) / self.n_rows` and `self.d_lon = (self.max_lon - self.min_lon) / self.n_cols` raise `ZeroDivisionError` when `n_rows` or `n_cols` is zero — reachable if a GDAL VRT clip produces an empty band or a corrupted raster reports zero extent. Guard both divisions: if `n_rows == 0` or `n_cols == 0`, raise `RuntimeError("DEM raster has zero rows/cols: {dem_path}")` before computing `d_lat` / `d_lon`.
