...
- Added a Windows mirror of `configure_macos_multiprocessing` / `find_macos_python_executable` in a new `windows_compat.py`. `_can_spawn` is shared between the two via `from .macos_compat import _can_spawn`. The Windows helper looks for `python.exe` in standalone and OSGeo4W-style bundle layouts (`<qgis>/python.exe`, `<qgis>/../apps/Python3X/python.exe`, `<qgis>/bin/python.exe`, etc.), validates each candidate by actually launching it under a prepared env (`PYTHONHOME=sys.prefix`), and honours the `NOWIRES_PYTHON_EXE` env var as an explicit override.
