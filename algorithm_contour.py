...
class ContourLinesAlgorithm(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        try:
            ...
            for tile_path in tile_paths:
                ...
                self._temp_files.append(fn_clip)
            ...
            feedback.pushInfo(
                "Contour lines generated: " + str(layer.featureCount())
            )
            ...
            if dem_output:
                ...
                if output_dem_layer.isValid():
                    queue_layer_for_loading(context, output_dem_layer, "NoWires DEM")
            ...
            if generate_elevation_map:
                ...
                if dem_layer.isValid():
                    ...
                    queue_layer_for_loading(context, dem_layer, "Elevation Overlay")
    ...
    def _smooth_contour_line(self, smoothing, feedback):
        ...
        if smoothing == "Low":
            ...
            self._temp_files.append(os.path.join(path, "dem_blur_7x7.vrt"))
            _make_blur_vrt(
                os.path.join(path, "dem_blur_7x7.vrt"),
                os.path.join(path, "dem.tif"),
                kernel_size=7,
            )
            # Blend 3x3 blur (B) with 7x7 blur (C) guided by TPI (A)
            _raster_calc(
                lambda A, B, C: A * B + (1 - A) * C,
                output_path=os.path.join(path, "merged_contour.tif"),
                nodata=-32768,
            ...
            self._temp_files.append(os.path.join(path, "dem_blur_13x13.vrt"))
            _make_blur_vrt(
                os.path.join(path, "dem_blur_13x13.vrt"),
                os.path.join(path, "dem.tif"),
                kernel_size=13,
            )
            # Blend 3x3 blur (B) with 13x13 blur (C) guided by TPI (A)
            _raster_calc(
                lambda A, B, C: A * B + (1 - A) * C,
                output_path=os.path.join(path, "merged_contour.tif"),
                nodata=-32768,
